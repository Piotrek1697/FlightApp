            ...
            override fun onQueryTextSubmit(query: String): Boolean {
                ...
                if (query != ""){
                    ...
                    if (hasLetters && hasNumbers){
                        mMap.clear()
                        val list = cordsList.filter {it.callsign == query.toUpperCase()} as MutableList<State>
                        if (list.size != 0)
                            putMarkersOnMap(mMap, list)
                        else
                            Toast.makeText(applicationContext, "There is no flights with number: ${query}", Toast.LENGTH_SHORT).show()
                    }else if(isCountry(query)){ //Searching by country
                        mMap.clear()
                        val list = cordsList.filter {it.origin_country.toLowerCase() == query.toLowerCase()} as MutableList<State>
                        if (list.size != 0)
                            putMarkersOnMap(mMap, list)
                        else
                            Toast.makeText(applicationContext, "There is no flights with country: ${query}", Toast.LENGTH_SHORT).show()
                    }else
                        Toast.makeText(applicationContext, "Wrong query string", Toast.LENGTH_SHORT).show()
                }
